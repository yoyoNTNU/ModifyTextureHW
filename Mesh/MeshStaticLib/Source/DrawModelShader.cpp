...
bool DrawModelShader::Init()
{
	...
	um4uvtLocation = GetUniformLocation("um4uvt");
	if (um4uvtLocation == -1)
	{
		puts("Get uniform loaction error: um4uvt");
		return false;
	}
	
	um4uvsLocation = GetUniformLocation("um4uvs");
	if (um4uvsLocation == -1)
	{
		puts("Get uniform loaction error: um4uvs");
		return false;
	}
...
void DrawModelShader::SetUVTranslateMat(const glm::mat4& mat)
{
	glUniformMatrix4fv(um4uvtLocation, 1, GL_FALSE, glm::value_ptr(mat));
}

void DrawModelShader::SetUVScaleMat(const glm::mat4& mat)
{
	glUniformMatrix4fv(um4uvsLocation, 1, GL_FALSE, glm::value_ptr(mat));
}